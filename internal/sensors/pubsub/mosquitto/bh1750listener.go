...
func (p MosquittoPubSub) NewBH1750Listener(ctx context.Context, listenerName string, subscription string) error {
	...
	var f mqtt.MessageHandler = func(client mqtt.Client, msg mqtt.Message) {
		...
		// unmashal payload
		sensor := &sensors.BH1750Measurement{}
		...
		if err == nil {
			err = p.persistence.CreateBH1750Measurement(ctx, sensor)
