...
func (client *DevicesClient) SendLampDevicePulseSetting(c *gin.Context) {
	ctx, cancel := context.WithTimeout(c, FIVE_MINUTES)
	defer cancel()

	var (
		req           request.SendLampDevicePulseSettingRequest
	...
	_, err := client.deviceSettingsClient.SendLampDevicePulseSetting(ctx, &devices_pb.SendLampDevicePulseSettingRequest{
		Mac:   req.Mac,
		Red:   *req.Red,
		Green: *req.Green,
		Blue:  *req.Blue,
